...
- **AI Recipe Generation**: When 2+ items are expiring (within 5 days), an "Expiring Items" banner appears on the Pantry screen. Users select ingredients and get AI-generated recipes using Gemini. Flow: RecipeSuggestion → RecipeResults → AIRecipeDetail → CookingMode → CookingComplete
- **Cooking Mode**: Full-screen step-by-step cooking instructions with swipeable navigation, progress dots, and metadata badges (duration, temperature). Accessed via "Start Cooking" button on recipe detail screen
- **Push Notifications**: Expiring item notifications using expo-notifications. Permission requested after first item added to pantry. Notifications scheduled for items expiring within 5 days
...
- `POST /api/generate-recipe` - Generates 2 AI recipes from expiring ingredients using Gemini. Body: `{ expiringIngredients, pantryItems, maxCookingTime? }`. Returns step-by-step instructions with structured output
